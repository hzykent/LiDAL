...
# Calculate softmax entropy for idth frame
def worker_func(id):
    ...
    frame_ent = np.mean(entropy(prob, axis=1))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description = 'Active selection in the frame level based on softmax entropy')
    ...
    save_folder = 'Processing_files/{}/frame_flag/{}/ENT'.format(args.dataset_name, args.model_name)
    if not os.path.exists(save_folder):
        os.makedirs(save_folder) 
    save_folder = 'Processing_files/{}/frame_flag/{}/ENT/{}r'.format(args.dataset_name, args.model_name, args.r_id)
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    ...
    for seq_id in train_split:
        ...
        if args.r_id == 1:
            ...
            frame_flag = np.load('Processing_files/{}/frame_flag/{}/ENT/{}r/{}.npy'.format(args.dataset_name, args.model_name, args.r_id - 1, seq_id))
        all_frame_flag = np.append(all_frame_flag, frame_flag)
        seq_offsets.append(seq_offsets[-1] + frame_flag.shape[0])
    
    # Softmax entropy for each frame in the dataset
    all_ents = np.zeros_like(all_frame_flag, dtype=np.float32)
    ...
    for seq_id in train_split:
        ...
        if args.r_id == 1:
            ...
            prob_files = sorted(glob.glob('Processing_files/{}/prob_map/{}/fr/ENT/{}r/{}/*.npy'.format(args.dataset_name, args.model_name, args.r_id - 1, seq_id)))
                
        # Calculate softmax entropy for each frame   
        # multi-processing
        ids = np.arange(len(prob_files))
        with Pool(processes=24, initializer=init_worker, initargs=(seq_id, prob_files)) as pool:
            frame_ents = pool.map(worker_func, ids)  
            frame_ents = np.array(frame_ents, dtype=np.float32)
        
        # Save
        all_ents = np.append(all_ents, frame_ents)
        

    # Select frames with the largest 1% ent in all sequences
    # Sort
    unlabeled_ids = np.where(all_frame_flag == False)[0]
    unlabeled_ents = all_ents[unlabeled_ids]
    # Select frames with the largest 1% ent
    num_add = round(0.01 * all_frame_flag.shape[0])
    selected_ids = np.argpartition(unlabeled_ents, -num_add)[-num_add:]
    ...
    for idx, seq_id in enumerate(train_split):
        ...
        np.save('Processing_files/{}/frame_flag/{}/ENT/{}r/{}.npy'.format(args.dataset_name, args.model_name, args.r_id, seq_id), frame_flag)
